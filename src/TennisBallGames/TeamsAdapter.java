...
public class TeamsAdapter {
    ...
    public void setStatus(String hTeam, String vTeam, int hScore, int vScore) throws SQLException {
        ...
        rs2 = stmt.executeQuery("SELECT * FROM Teams WHERE TeamName = '"+ vTeam + "'");


            if (hScore > vScore) {
                win = rs.getInt("Wins") + 1;
                lose = rs2.getInt("Losses") + 1;
                stmt.executeUpdate("UPDATE Teams SET Wins = " + win + " WHERE TeamName = '" + hTeam + "'");
                stmt.executeUpdate("UPDATE Teams SET Losses = " + lose + " WHERE TeamName = '" + vTeam + "'");
            } else if (vScore > hScore) {
                win = rs2.getInt("Wins") + 1;
                lose = rs.getInt("Losses") + 1;
                stmt.executeUpdate("UPDATE Teams SET Losses = " + lose + " WHERE TeamName = '" + hTeam + "'");
                stmt.executeUpdate("UPDATE Teams SET Wins = " + win + " WHERE TeamName = '" + vTeam + "'");
            } else {
                tie1 = rs.getInt("Ties") + 1;
                tie2 = rs2.getInt("Ties") + 1;
                stmt.executeUpdate("UPDATE Teams SET Ties = " + tie1 + " WHERE TeamName = '" + hTeam + "'");
                stmt.executeUpdate("UPDATE Teams SET Ties = " + tie2 + " WHERE TeamName = '" + vTeam + "'");
            }
